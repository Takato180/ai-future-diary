...
-  **画像アップロード**: ユーザーが撮影した写真をアップロード可能
-  **デュアル画像システム**: AI生成画像とアップロード画像の両方を保存・表示（アップロード優先）
...
-  **ビデオ生成**: 7日間連続日記達成でVeo 3.0による特別動画生成
-  **イントロ動画**: ログイン時の歓迎動画表示機能
...
**主な機能:**
- DiaryEntry、User データモデル定義（デュアル画像フィールド対応）
- CRUD操作（作成、読み取り、更新）
- 月別・年別エントリ取得機能
- ユーザー認証・管理機能
- 表紙画像生成・更新機能
- 接続状態確認機能
- AI生成画像とアップロード画像の分離管理
...
**主な機能:**
- **エントリ保存** (`POST /diary/entries/{date}`): 日記エントリの保存・更新（デュアル画像対応）
- **エントリ取得** (`GET /diary/entries/{date}`): 特定日の日記取得
- **月別一覧** (`GET /diary/entries?month=YYYY-MM`): 月別日記一覧
- **年別一覧** (`GET /diary/entries/year/{year}`): 年別日記一覧
- **振り返りサマリー** (`POST /diary/entries/{date}/diff`): AIによる差分分析
- **7日間ストリーク確認** (`GET /diary/streak-check`): 連続記録チェック
...
- **画像アップロード対応**: ユーザー写真のCloud Storage保存
...
- デュアル画像表示（AI生成 + アップロード、アップロード優先）
- データ保存・読み込み機能（ユーザー別、キャッシュ最適化）
- 振り返りサマリー表示
- レスポンシブなノートブック風デザイン
...
- 画像生成API呼び出し
- 画像アップロード機能（Cloud Storage連携）
- 日記データCRUD操作（ユーザー別、デュアル画像対応）
- ユーザー認証API呼び出し
- ビデオ生成・ストリーク確認API
- JWT認証ヘッダー処理
- エラーハンドリング
...
#### Web/src/components/IntroPlayer.tsx
ログイン時イントロ動画表示コンポーネント。

**主な機能:**
- default_login.mp4の全画面表示
- セッション制限なしでの動画再生
- レスポンシブ動画プレイヤー

#### その他の新規コンポーネント
- **src/videogen.py**: Veo 3.0による動画生成機能
- **src/intro.py**: イントロ動画管理機能
